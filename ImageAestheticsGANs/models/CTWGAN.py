...
class Generator(nn.Module):
    def __init__(self, n_classes):
        ...
        self.emb = nn.Embedding(n_classes, n_classes*n_classes)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.block1 = nn.Sequential(
            # ResidualBlock(n_output, n_output, 3, resample='down', bn=False, spatial_dim=128),  # 128 x 8 x 8
            # ResidualBlock(n_output, n_output, 3, resample='down', bn=False, spatial_dim=64),
            ResidualBlock(n_output, n_output, 3, resample='down', bn=False, spatial_dim=32),
            ResidualBlock(n_output, n_output, 3, resample='down', bn=False, spatial_dim=16),
        )
...
if __name__ == "__main__":
    ...
    image = read_image_transform('F:\Projects\Disertatie\ImageAestheticsGANs\AADB\ImageAesthetics_ECCV2016\datasetImages_warp256\\farm1_255_19452343093_8ee7e5e375_b.jpg')
    image = image.unsqueeze(0).to('cuda')

    cuda = torch.cuda.is_available()
    Tensor = torch.cuda.FloatTensor if cuda else torch.FloatTensor
    noise_fixed = Variable(Tensor(1, 128, 1, 1).normal_(0, 1), requires_grad=False)
    noise_labels = Variable(torch.randint(2, (1, 8, 1, 1)), requires_grad=False).to('cuda')

    # print(image.shape)
    # print(noise_fixed.shape)
    # print(noise_labels.shape)

    generator = Generator(8).to('cuda')
    discriminator = Discriminator().to('cuda')

    # critic_value = discriminator(image)
    # print(critic_value)

    img = generator(noise_fixed, noise_labels)
    # print(img.shape)